...
fn get_note_query(
    ...
    format!(
        ...
        SELECT
            ...
            revision.note_inner,
            revision.parent,
            array_remove(array_agg(DISTINCT revision1.id), NULL) AS branches,
            revision.prev,
            array_remove(array_agg(DISTINCT revision2.id), NULL) AS next,
            revision.referents,
            array_remove(array_agg(DISTINCT revision3.id), NULL) AS "references",
